...
public class LevelTestActivity extends AppCompatActivity {
    ...
    /*
     * 레벨 테스트용 문제 불러오기
     * Easy / Normal / Hard 문제를 모두 가져온 뒤 중복 제거 후 Knapsack 알고리즘 적용
     */
    private void fetchQuestionsAndStartTest(int subjectId) {
        // key = quizId, value = QuizQuestion (중복 자동 제거)
        ...
        repository.getQuizQuestionsFromFirestore(
                ...
                new QuizRepository.OnQuestionsFetchedListener() {
                    ...
                    public void onSuccess(List<QuizQuestion> easyQuestions) {
                        ...
                        repository.getQuizQuestionsFromFirestore(
                                ...
                                new QuizRepository.OnQuestionsFetchedListener() {
                                        ...
                                        repository.getQuizQuestionsFromFirestore(
                                                ...
                                                new QuizRepository.OnQuestionsFetchedListener() {
                                                    ...
                                                    public void onSuccess(List<QuizQuestion> hardQuestions) {
                                                        ...
                                                        Log.d("LevelTest", "중복 제거 후 문제 수 = " + mergedQuestions.size());

                                                        for (QuizQuestion q : mergedQuestions) {
                                                            Log.d("LevelTest", "문제: " + q.getQuestion() + " / 난이도: " + q.getDifficultyLevel());
                                                        }

                                                        /*
                                                         * 현재 목표(DB에 문제 추가 후에 수정 해야함)‼️‼️‼️:
                                                         * Easy 5개 (50점) / Hard 2개 (60점) -> 총 110점
                                                         */
                                                        testQuestions = KnapsackQuizSelector.selectQuestions(
                                                                mergedQuestions,
                                                                110,
                                                                5,
                                                                0,
                                                        ...
                                                        Log.d("LevelTest", "최종 선택 문제 수 = " + testQuestions.size());

                                                        if (testQuestions.size() < TEST_TOTAL_COUNT) {
                                                            showErrorDialogAndGoBack("조건에 맞는 문제가 부족합니다.");
                                                            return;
                                                        }
                                                    ...
                                                    public void onFailure(Exception e) {
                                                        showErrorDialogAndGoBack("Hard 문제를 불러오지 못했습니다.\n" + e.getMessage());
                                                    }
                                                }
                                        );
                                    }
                                    ...
                                    @Override
                                    public void onFailure(Exception e) {
                                        showErrorDialogAndGoBack("Normal 문제를 불러오지 못했습니다.\n" + e.getMessage());
                                    }
                                }
                        );
                    }
                    ...
                    @Override
                    public void onFailure(Exception e) {
                        showErrorDialogAndGoBack("Easy 문제를 불러오지 못했습니다.\n" + e.getMessage());
                    }
                }
        );
    }
    ...
    private void showQuestion(int index) {
        ...
        tvQuizQuestion.setText("Q" + (index + 1) + ". " + currentQuiz.getQuestion());
    ...
    private void finishLevelTest(int totalSolved, int correct) {
        ...
        // Firestore에 레벨 데이터 저장
        db.collection("users")
                .document(uid)
                .update(updates)
                .addOnSuccessListener(aVoid -> {

                    // 파일명 뒤에 _uid를 붙여 이 계정 전용 로컬 SharedPreferences 파일에 저장
    ...
    private void moveToMain() {
        ...
        // 기존에 쌓여있던 모든 액티비티 스택을 완전히 비우고 새 액티비티로 시작 (중복 호출 제거 및 최적화)
    ...
    private void updateOptionBackgrounds() {
        LT_option1.setBackgroundResource(
                LT_option1.isChecked() ? R.drawable.bg_quiz_option_selected : R.drawable.bg_option_default
        );
        LT_option2.setBackgroundResource(
                LT_option2.isChecked() ? R.drawable.bg_quiz_option_selected : R.drawable.bg_option_default
        );
        LT_option3.setBackgroundResource(
                LT_option3.isChecked() ? R.drawable.bg_quiz_option_selected : R.drawable.bg_option_default
        );
        LT_option4.setBackgroundResource(
                LT_option4.isChecked() ? R.drawable.bg_quiz_option_selected : R.drawable.bg_option_default
        );
    }
}